...
void startAddPatientFunction(FILE* patientFile, Patient *addPatient){
    ...
    FILE* medicineFileCheck = openMedicineFileRead();
    Medicine* medcineArray = (Medicine*)malloc(MAX_MEDCINES * sizeof(Medicine));
    int count2 = populateMedicineArray(medicineFileCheck, medcineArray);
    ...
    if (strcmp(userAnswer2, "yes") == 0) {
        ...
        while (count < MAX_PRESCRIPTIONS) {
            ...
            for (int i = 0; i < count2; ++i) {
                if(medcineArray[i].ndc == atoll(userAnswerNDC)){
                    medicineFound = 1;
                    break;
                }
            }

            fclose(medicineFileCheck);

            if(medicineFound){
                addPatient->prescriptions[count] = atoll(userAnswerNDC);
                count++;
            }else{
                printf("The following NDC is not registered in the database. \n");
            }
